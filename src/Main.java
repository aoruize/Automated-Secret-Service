    ...
    public synchronized void serialEvent(SerialPortEvent oEvent) {
        if (oEvent.getEventType() == SerialPortEvent.DATA_AVAILABLE) {
            try {
                ...
                double xDiff = Math.abs(x - prevX);
                double yDiff = Math.abs(y - prevY);
                double zDiff = Math.abs(z - prevZ);
                //System.out.println(x + " " + prevX + " " + y + " " + prevY + " " + z + " " + prevZ);
                //System.out.println(xDiff + " " + yDiff + " " + zDiff);
                if ((zDiff > 0.85 || yDiff > 0.85 || xDiff > 0.85) && !Hotkey.pause) {
                    java.awt.Desktop.getDesktop().browse(new URI ("www.drive.google.com"));
                    Hotkey.pause = true;
                    pauseTime = System.nanoTime() / 1000000000.0;
                }
                prevZ = z;
                prevX = x;
                prevY = y;
